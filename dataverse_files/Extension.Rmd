...
parallel.trends_100_graphic

for (i in helper){
  
  if (i <= helper[length(helper)] - 2){
    
    parallel.trends_100_graphic <- parallel.trends_100_graphic + 
    
      geom_segment(aes(x = value[i], y = WT[i], xend = value[i + 1], yend = WT[i + 1]))
    ...
    parallel.trends_100_graphic <- parallel.trends_100_graphic + 
      ...
      geom_segment(aes(x = value[i], y = WT[i], xend = value[length(parallel.trends_100$WT)], yend = WT[length(parallel.trends_100$WT)]))
